...
/// A parseable manifest with an empty resource list, for manually built containers.
...
#[test]
fn generic_writer_propagates_final_flush_failure() {
    ...
    let result = finish_fixture(WaczWriter::new(FlushFailure(Cursor::new(Vec::new()))), &[]);
    assert!(
        matches!(result, Err(writer::Error::Io(error)) if error.to_string() == "deliberate flush failure")
    );
}
...
#[test]
fn publication_never_overwrites_a_competing_output() -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempfile::tempdir()?;
    let path = directory.path().join("test.wacz");
    let mut writer = WaczWriter::create(&path)?;
    writer.add_warc("data.warc", warc_bytes()?.as_slice())?;
    writer.add_pages(&PageListHeader::default(), [])?;
    ...
    assert!(writer.finish(DataPackageBuilder::default()).is_err());
    assert_eq!(std::fs::read(&path)?, b"other writer");
    assert_eq!(std::fs::read_dir(directory.path())?.count(), 1);
    Ok(())
}

#[test]
fn dropping_or_failing_file_writer_cleans_up_staging() -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempfile::tempdir()?;
    let path = directory.path().join("unfinished.wacz");
    let pending = writer::WaczFileWriter::create(&path)?;
    assert!(!path.exists());
    assert_eq!(std::fs::read_dir(directory.path())?.count(), 1);
    drop(pending);
    assert_eq!(std::fs::read_dir(directory.path())?.count(), 0);
    ...
    let pending = writer::WaczFileWriter::create(&path)?;
    assert!(matches!(
        pending.finish(DataPackageBuilder::default()),
...
/// The convenience constructor uses the `py-wacz` block size of 1024 lines.
#[test]
...
fn validate_reports_content_problems() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (path, kind) in [
        ...
        assert!(
            content
                .iter()
                .any(|problem| problem.path == path && problem.kind == kind)
        );
    }
