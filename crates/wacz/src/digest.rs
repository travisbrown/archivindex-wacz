...
/// The `sha256:<hex>` digest encoding used in WACZ manifests.
///
/// Uppercase hexadecimal is accepted on input but never written.
struct Encoding;
...
impl<'de> serde::de::Deserialize<'de> for Sha256Digest {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        archivindex_serde::from_str(deserializer, "sha256 digest string")
    }
}
