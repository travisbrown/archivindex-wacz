...
//!
//! A WACZ manifest is a [Frictionless Data Package][data-package] descriptor listing archive
//! resources with their sizes and SHA-256 digests. The manifest and digest file are excluded from
//! this list. The digest file hashes the serialized manifest and may include a signature.
//!
...
//! properties must match their declared types. Parsing alone does not check metadata
//! constraints; use [`DataPackage::constraint_errors`] or
//! [`crate::io::read::WaczReader::validate`]. The validator also accepts recognized nonstandard
//! dates so it can report them alongside other problems.
...
pub enum ConstraintError {
    ...
    ContributorTitle,
    /// A contributor's role is outside the recommended vocabulary enforced by this crate.
...
/// Source metadata with a required title.
///
/// [`Source`] also represents input that omits the title.
...
/// License metadata with a required name or path.
///
/// License identifiers and paths are stored without checking their validity.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// The contribution roles recommended by the Data Package specification.
...
/// Contributor metadata with a required display name.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Collect violations of the metadata constraints checked by this crate.
...
pub struct DataPackage<'a> {
    /// The data package profile identifier; WACZ requires [`PROFILE`].
    #[serde(borrow)]
    ...
    #[serde(
        default,
        with = "archivindex_serde::borrowable_str::seq",
...
impl DataPackage<'_> {
    /// Check the package name, required source and contributor titles, license identities, and
    /// extension property collisions, including those in resource metadata.
    ///
    /// Contributor roles are restricted to [`ContributorRole`], the specification's recommended
    /// vocabulary. Requirements involving the archive's other files are checked by
    /// [`crate::io::read::validate`].
    #[must_use]
...
/// and `resources`) when it finishes the archive. It also supplies defaults for `created`,
/// `modified`, and `software` when those properties are not set here.
...
impl DataPackageBuilder {
    ...
    ///
    /// The Data Package specification restricts names to lowercase characters from `a-z0-9._-`.
    ...
    optional_string_setter!(
        software,
        software,
        "Describe the software that created the package."
    );
...
pub struct DataPackageDigest<'a> {
    /// The manifest path; WACZ requires `datapackage.json`.
    #[serde(borrow)]
    pub path: Cow<'a, str>,
...
///
/// The specification requires `title`. Parsing permits its absence so validation can report it.
...
///
/// The specification requires `title` and recommends the roles in [`ContributorRole`], with
/// `contributor` as the default. Parsing retains missing titles and arbitrary roles; metadata
/// validation requires a title and enforces the recommended roles.
#[derive(Clone, Debug, Default, Eq, PartialEq, ToStatic, serde::Deserialize, serde::Serialize)]
// Every field is optional, so no `#[serde(borrow)]` field ties the deserializer's input lifetime to
// `'a`; state the bound explicitly to allow borrowing from the input.
#[serde(bound(deserialize = "'de: 'a"))]
...
impl Contributor<'_> {
    pub(crate) fn push_constraint_errors(&self, errors: &mut Vec<ConstraintError>) {
        ...
        if let Some(role) = &self.role
            && ContributorRole::parse(role).is_none()
        {
...
mod tests {
    ...
    #[test]
    fn authored_contributor_roles_validate_with_their_wire_spelling() {
        for (role, spelling) in [
            (ContributorRole::Author, "author"),
            (ContributorRole::Publisher, "publisher"),
            (ContributorRole::Maintainer, "maintainer"),
            (ContributorRole::Wrangler, "wrangler"),
            (ContributorRole::Contributor, "contributor"),
        ] {
            let contributor = Contributor::from(ContributorMetadata::new("Example").role(role));
            let json = serde_json::to_string(&contributor).unwrap();
            let parsed: Contributor<'_> = serde_json::from_str(&json).unwrap();
            assert_eq!(parsed.role.as_deref(), Some(spelling));
            assert_eq!(ContributorRole::parse(spelling), Some(role));
            let mut errors = Vec::new();
            parsed.push_constraint_errors(&mut errors);
            assert!(errors.is_empty(), "{errors:?}");
        }
    }

    #[test]
    fn unknown_contributor_roles_round_trip_and_report_constraints() {
        for role in ["editor", "Author", "", "author "] {
            let json = serde_json::json!({"title": "Example", "role": role});
            let text = json.to_string();
            let parsed: Contributor<'_> = serde_json::from_str(&text).unwrap();
            assert_eq!(parsed.role.as_deref(), Some(role));
            assert_eq!(serde_json::to_value(&parsed).unwrap(), json);
            let mut errors = Vec::new();
            parsed.push_constraint_errors(&mut errors);
            assert_eq!(errors, [ConstraintError::ContributorRole(role.to_owned())]);
        }
    }
