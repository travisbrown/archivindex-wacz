...
//! specification. Unmodeled properties (for example `data` or `schema`) are preserved in
//! [`Resource::extra`].
...
pub struct Resource<'a> {
    /// A unique resource identifier, usually the file name, using lowercase `a-z0-9._-`.
    #[serde(borrow)]
    pub name: Cow<'a, str>,
...
impl<'a> Resource<'a> {
    /// Create a resource with the required properties and no optional metadata.
    pub fn new(
