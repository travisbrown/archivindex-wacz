...
//! separating layout problems, metadata errors, and digest mismatches. The layout, manifest, and
//! signature layers always run, reading ZIP headers and the two metadata files. Optional fixity,
//! content, and index checks read archive content; select them through [`ValidationOptions`].
//!
...
/// Optional fixity, content, and index checks for [`WaczReader::validate`].
///
/// Layout, manifest, and signature checks always run. The default enables no additional checks.
...
impl ValidationOptions {
    ...
    ///
    /// This also enables content checking, since resolving records is meaningful only for an
    /// index that parses.
    #[must_use]
...
pub struct ValidationReport {
    /// Missing required members, duplicate names, invalid paths, and disallowed ZIP compression.
    ...
    /// The outcome of checking declared digests and sizes, if the fixity layer ran and the manifest
...
pub enum LayoutProblem {
    ...
    /// A member under `indexes/` is neither a plain CDXJ index nor part of a complete `ZipNum`
    /// pair.
    ...
    MissingPages,
    /// There are no recognized WARC members under `archive/`.
    #[error("no WARC members under archive/")]
    NoWarcMembers,
    /// There are no recognized CDXJ data members under `indexes/`.
...
pub enum ManifestProblem {
    ...
    /// A ZIP member other than the manifest and digest file is not listed as a resource, so fixity
    /// checking cannot cover it.
...
///
/// Only the first problem in each member is reported, keeping the report bounded when parse
/// failures cascade.
#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
...
pub enum IndexProblem {
    ...
    /// A `ZipNum` block cannot be read, does not match its declared digest, or does not decompress.
...
impl<R: Read + Seek> WaczReader<R> {
    ...
    ///
    /// When the manifest is missing or unparseable, the layout or manifest layer reports it and the
    /// fixity layer is skipped even if selected, since there are no declared digests to check.
    pub fn validate(&mut self, options: ValidationOptions) -> Result<ValidationReport, Error> {
        ...
        // Repair recognized date formats so their errors do not prevent other manifest checks.
    ...
    fn compression_problems(&mut self) -> Vec<LayoutProblem> {
        ...
        // Reading by index gives each member's name and compression method together, which a shared
        // borrow of the central directory cannot provide.
    ...
    fn index_content_problem(&mut self, path: &str) -> Option<ContentProblem> {
        ...
        for (index, item) in items.enumerate() {
            match item {
                Ok(item) => {
                    if let Err(error) =
                        archivindex_cdx::format::cdxj::ConformingFields::try_from(&item.fields)
                    {
                        return Some(ContentProblem::new(
                            path,
