...
pub enum Error {
    ...
    /// Decompressed content exceeds the byte limit.
    #[error("member {path} expands past the {limit} byte decompression limit")]
    DecompressedTooLarge {
        /// The member path.
        path: String,
        /// The maximum number of decompressed bytes allowed.
        limit: u64,
    },
...
/// Whether manifest resources match their declared sizes and SHA-256 digests.
...
pub struct Fixity {
    ...
    /// Resource paths whose sizes or digests differ, plus the manifest if its digest check fails.
...
impl<R: Read + Seek> WaczReader<R> {
    ...
    /// File paths under `archive/`, in unspecified order, without checking their extensions.
    pub fn warc_paths(&self) -> impl Iterator<Item = &str> {
        self.paths_under(ARCHIVE_PREFIX)
    }

    /// File paths under `indexes/`, in unspecified order, without checking their extensions.
    ...
    ///
    /// Missing resources, size or digest mismatches, and `InvalidData` errors while reading
    /// resource contents are reported in the result. A digest file that cannot be parsed or does
    /// not name the manifest is also reported there. A missing or unparseable manifest and other
    /// read errors abort the check.
    ///
    ...
    /// Read a complete member, removing ZIP compression but retaining any gzip encoding.
...
/// The maximum decoded size of a gzip capture range or `ZipNum` block.
///
/// A gzip stream declares nothing about its decompressed size, so anything past this ceiling is
/// rejected rather than silently truncated. Whole-member reads do not use this limit.
