...
impl<W: Write + Seek> WaczWriter<W> {
    /// Add a custom resource outside `archive/`, `indexes/`, and `pages/`, tracking its digest
    /// and size.
...
///
/// The conventional `extraPages.jsonl` file name needs a lowercase resource name. Its path remains
/// unchanged; only the manifest identifier is normalized.
