...
impl<W: Write + Seek> WaczWriter<W> {
    /// Sort and deduplicate CDXJ items, then write the configured plain or `ZipNum` index.
    ///
    /// Sorting and deduplication use complete serialized lines.
    ///
    /// # Errors
    ///
    ...
    /// for missing or colliding CDXJ fields, and [`Error::UnsortedIndex`] for the first line
    /// that sorts before the line preceding it.
    ...
    fn add_zipnum_stream(
        ...
        self.add_member(&data_path, data_options, |writer| {
            ...
            // Each block is one gzip member encoded into a reused buffer, digested and written from
            // memory; the line buffer is reused across the whole stream.
...
///
/// Lines are sorted and deduplicated in batches written to temporary files, then merged with a
/// limited number of files open at once. Pass the spool to [`WaczWriter::add_spooled_index`].
...
impl IndexSpool {
    ...
    ///
    /// Temporary files are created when buffered lines reach the batch size or the spool is
    /// consumed by a writer.
    #[must_use]
    pub fn new() -> Self {
...
fn merge_runs(runs: &[Run], sink: File) -> Result<File, std::io::Error> {
    ...
    // Each popped line's buffer is reused for the next line of the same run; an emitted line swaps
    // buffers with `previous` instead of being copied.
