...
//!
//! [`PageListReader`] reads [`Page`] entries with RFC 3339 timestamps. [`RawPageListReader`]
//! reads [`RawPage`] entries that retain the timestamp text, allowing callers to handle
//! nonstandard dates.
...
use archivindex_lines::{LineContext, LineContextRef, Lines};
...
pub struct PageListHeader<'a> {
    /// The format identifier; page-list readers and writers require [`FORMAT`].
    #[serde(borrow)]
...
impl<R: BufRead> Iterator for PageListReader<R> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        Some(self.raw.next_located()?.and_then(|(context, page)| {
            Page::try_from(page).map_err(|source| Error::InvalidTimestamp {
                source,
...
/// # Errors
///
/// Fails if the header or a page has invalid properties, serialization fails, or the stream cannot
/// be written.
...
impl<W: Write> PageListWriter<W> {
    ...
    /// Return the underlying writer without flushing it.
