...
use archivindex_wacz::io::write::{IndexFormat, WaczFileWriter, WriterConfig};
...
pub struct Capture<'a> {
    ...
    pub status: u16,
    /// The decoded entity body, or stored body bytes when decoding fails. Empty for revisits.
    pub payload: &'a [u8],
...
/// Why a capture could not be indexed.
...
pub struct ConversionSummary {
    /// The number of WARC records copied into the package.
    pub records: usize,
    /// The number of indexed HTTP `response` and `revisit` records, before index deduplication.
    pub captures: usize,
    /// The number of entries added across both WACZ page lists.
    pub pages: usize,
...
///
/// Records are copied byte for byte into a random-access WARC member; only gzip framing may change.
/// Only record types needed for package metadata and indexing are parsed semantically. Missing
/// payload digests are computed for the index without changing the records. HTTP captures without
/// a usable status or digest remain in the WARC but are not indexed and produce a
/// [`ConversionWarning`].
///
/// Metadata linked by `WARC-Refers-To` or `WARC-Concurrent-To` supplies page properties. A nonempty
/// `title` overrides the optional [`PageTitleGenerator`], and `pageUrl` overrides the capture URL.
/// If the first `warcinfo` declares `pageList: metadata`, only indexed captures with a linked
/// `pageUrl` enter the page lists; otherwise, every indexed capture gets a page entry. Entries with
/// linked `via` metadata go in `pages/extraPages.jsonl`; the rest go in `pages/pages.jsonl`.
...
impl<'a> WarcToWacz<'a> {
    ...
    ///
    /// A nonempty title recorded in linked WARC metadata replaces the generated title.
    #[must_use]
    ...
    /// Set the gzip compression level for packaged WARC records and `ZipNum` index blocks.
    ///
    /// Levels range from 0 (no compression) through 9 (best compression), and default to 6. This
    /// setting applies to gzip WARC output (including recompressed input) and compressed indexes.
    ...
    ///
    /// Levels range from 1 through 264 and default to 6. Levels up to 9 use `miniz_oxide`, while
    /// higher levels use Zopfli. WARC and gzip-compressed members use ZIP `STORE` and are
    /// unaffected.
    ...
    fn convert<R: BufRead>(
        ...
        let mut wacz =
            WaczFileWriter::create_with_config(&self.output, self.writer_config.clone())?;
...
/// The record types parsed semantically during conversion; all others are copied as raw records.
...
impl PackageInfo {
    fn inspect(&mut self, record: &Record) {
        ...
        // A crawl session names the collection its WARC is part of rather than titling it.
        self.title = fields
            .get(&WarcinfoField::Dcmi(DcmiTerm::Title))
            .or_else(|| fields.get(&WarcinfoField::Dcmi(DcmiTerm::IsPartOf)))
            .map(str::to_owned);
...
/// Build an index entry and page draft for an HTTP capture at the byte range in `written`.
///
/// Returns `Ok(None)` for other record types or targets that cannot be canonicalized as HTTP(S)
/// URLs. An error identifies a missing status or payload digest.
fn capture_parts(
    record: &Record,
