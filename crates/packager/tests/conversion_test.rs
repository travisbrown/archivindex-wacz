...
fn index_fields_follow_cdxj_conventions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let items = reader
        ...
        .collect::<Result<Vec<_>, _>>()?;
    for item in &items {
        assert!(archivindex_cdx::format::cdxj::ConformingItem::try_from(item).is_ok());
    }
    let fields = |path: &str| {
...
fn records_are_copied_verbatim() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut request = Record::<NoExtension>::request("https://example.com/", date)?
        .body(b"GET / HTTP/1.1\r\n\r\n".to_vec())?
        .into_raw()?;
    // Without the mandatory `WARC-Date` the request is not a valid semantic record, but it is still
    // a well-formed raw record, and request records are never parsed.
    request
        .header
        .headers
