...
mod tests {
    ...
    fn a_resumed_run_at_page_zero_probes_the_endpoint_itself() {
        let processor = ArchiveProcessor::resume(site(), before(), Endpoint::Media, 0, None);
    ...
    fn a_carried_page_count_makes_not_modified_responses_resumable() {
        let mut processor = ArchiveProcessor::resume(site(), before(), Endpoint::Media, 0, Some(2));

        let probe = inspect(&mut processor, &endpoint_url(Endpoint::Media), NOT_MODIFIED);
        assert_eq!(probe.links, [page_url(Endpoint::Media, 1)]);
        assert_eq!(
            processor.checkpoint(),
            Checkpoint::Resume {
                endpoint: Endpoint::Media,
        ...
        let page = inspect(&mut processor, &page_url(Endpoint::Media, 1), NOT_MODIFIED);
        assert_eq!(page.links, [page_url(Endpoint::Media, 2)]);
        assert_eq!(
            processor.checkpoint(),
            Checkpoint::Resume {
                endpoint: Endpoint::Media,
