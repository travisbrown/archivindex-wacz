...
/// A package converted from a WARC, kept with the temporary directory both live in.
struct Package {
...
/// Convert an in-memory WARC, gzip-compressed or plain as its first bytes say, to a package.
...
/// Assert that an index entry's offset and length frame exactly one complete record within the
/// WARC member (one gzip member, decompressible on its own, when the member is compressed), that
...
fn packages_a_crawl_session_with_extra_pages() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // to /about and /missing, which are crawled as discoveries.
    let (port, server) = serve(5)?;
    let seeds = [
        format!("http://127.0.0.1:{port}/"),
        format!("http://127.0.0.1:{port}/redirect"),
...
fn packages_server_not_modified_revisits() -> Result<(), Box<dyn std::error::Error>> {
    let (port, server) = serve_with(3, |head| (respond_versioned(head, 2), ()))?;
