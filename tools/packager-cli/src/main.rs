...
///
/// Log non-fatal source problems as warnings and use exit status 1 when any are found.
fn warc_to_wacz(
...
#[derive(Debug, Parser)]
#[command(name = "archivindex-packager", version, author)]
struct Opts {
    #[command(flatten)]
    verbosity: Verbosity,
    #[command(subcommand)]
...
struct WarcToWaczOptions {
    ...
    #[arg(long)]
    output: PathBuf,
    /// Write a gzip-compressed CDXJ index with a block summary.
    #[arg(long)]
    compressed_index: bool,
    /// Gzip a plain input WARC, compressing each record independently for random access.
    #[arg(long)]
    gzip_warc: bool,
    /// Gzip compression level for WARC records and compressed index blocks (0-9).
    #[arg(long, default_value_t = 6, value_parser = clap::value_parser!(u32).range(0..=9))]
    gzip_compression_level: u32,
    /// ZIP DEFLATE level for metadata and plain indexes (1-264).
    #[arg(long, default_value_t = 6, value_parser = clap::value_parser!(u32).range(1..=264))]
